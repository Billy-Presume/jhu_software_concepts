...
def home():
    ...
    """
    # load_if_first_time()  # Uncomment if data should auto-load on app start.

    questions: list[tuple[str, list[str], list[Any]]] = []

    # 1. Count of entries for Fall 2025
    fall_2025_count = safe_fetch_query("SELECT COUNT(*) FROM applicants WHERE term = 'Fall 2025'"
                                      )[0][0]
    questions.append(
    ...
    intl = safe_fetch_query(
        "SELECT COUNT(*) FROM applicants WHERE us_or_international NOT IN ('American', 'Other')"
    )[0][0]
    total = safe_fetch_query("SELECT COUNT(*) FROM applicants")[0][0]
    ...
    avg_gpa_american = safe_fetch_query(
        """
    ...
    accepted_count = safe_fetch_query(
        """
    ...
    # 6. GPA of accepted Fall 2025 applicants
    accepted_gpa = safe_fetch_query(
        """
        SELECT ROUND(AVG(gpa)::NUMERIC, 2) FROM applicants 
    ...
    # 7. JHU CS Masters applicants
    jhu_cs_count = safe_fetch_query(
        """
        SELECT COUNT(*) FROM applicants 
